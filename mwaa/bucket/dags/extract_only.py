...
with DAG(
    ...
    ref_dt = "{{ execution_date.in_timezone('Asia/Seoul').strftime('%Y-%m-%dT%H:%M:%S') }}"
    ...
    end_datetime = "{{ execution_date.in_timezone('Asia/Seoul').strftime('%Y-%m-%dT%H:%M:%S') }}"
    ...
    for car_info in CARS.items():
        ...
        for community in COMMUNITIES:
            ...
            extract_task = create_extract_task(
                dag, community, car_id, keywords, ref_date, batch, start_datetime, end_datetime
            )
